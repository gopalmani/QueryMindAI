...
async function sessionToken(): Promise<string> {
  ...
  if (!response.ok)
    throw new ApiError('Could not create a secure workspace session', response.status);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const validation = Array.isArray(body.detail)
        ...
        : body.detail;
      throw new ApiError(
        body.error?.message || validation || 'Request failed',
        response.status,
...
export const api = {
  ...
  createConnection: (body: Record<string, unknown>) =>
    request<SavedConnection>('/connections', { method: 'POST', body: JSON.stringify(body) }, true),
  testConnection: (id: string) =>
    request<{ status: string; read_only: boolean | null; warnings: string[] }>(
      `/connections/${id}/test`,
      { method: 'POST' },
      true
    ),
  ...
  refreshSchema: (id: string) =>
    request<CatalogResponse>(`/connections/${id}/refresh-schema`, { method: 'POST' }, true),
  connectionSchema: (id: string) =>
    request<CatalogResponse>(`/connections/${id}/schema`, undefined, true),
  generateQuery: (connection_id: string, question: string) =>
    request<QueryDraft>(
      '/queries/generate',
      { method: 'POST', body: JSON.stringify({ connection_id, question }) },
      true
    ),
  executeDraft: (draft_id: string) =>
    request<QueryExecution>(
      '/queries/execute',
      { method: 'POST', body: JSON.stringify({ draft_id }) },
      true
    ),
  saveExecutedAsVerified: (queryId: number) =>
    request<{ id: number; status: string }>(
      `/queries/${queryId}/verified-example`,
      { method: 'POST' },
      true
    ),
  history: () => request<{ items: QueryHistoryItem[] }>('/queries/history', undefined, true),
};
